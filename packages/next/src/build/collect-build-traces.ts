...
export async function collectBuildTraces({
  ...
  await nextBuildSpan
    ...
    .traceAsyncFn(async () => {
      ...
      // worker files.
      const isOutputStandalone = config.output === 'standalone'
      const isOutputBun = config.output === 'bun'

      // Bun's output format is very similar to `standalone` and a lot
      // of the behaviour overlaps
      const isConsideredStandalone = isOutputStandalone || isOutputBun
